...
- After editing QML, build the target and run the QML load verifier against the active app bundle. The verifier launches through macOS LaunchServices because directly spawning an AppKit process from Codex can fail before QML is reached. For Qt Creator debug runs in this repo, verify:
...
- If the verifier fails, inspect its QML error output, fix the runtime QML error, and repeat the check. The core tests are useful but do not cover QML component loading.
